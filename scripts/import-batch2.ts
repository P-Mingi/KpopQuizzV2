...
// crypto not needed - batch2 reuses existing creators
...
const CREATOR_MAP: Record<string, string> = {
  // joonified - BTS quizzes
  'BTS Love Yourself era quiz': 'joonified',
  'BTS Map of the Soul era': 'joonified',
  'BTS Wings era deep dive': 'joonified',
  'BTS members real names - complete test': 'joonified',
  'K-pop BT21 characters quiz': 'joonified',

  // pinkvelvet - BLACKPINK quizzes
  'BLACKPINK solo careers deep dive': 'pinkvelvet',
  'BLACKPINK world records and achievements': 'pinkvelvet',
  "BLACKPINK 'THE ALBUM' quiz": 'pinkvelvet',

  // skzrealm - Stray Kids + general
  'Stray Kids NOEASY to MAXIDENT era': 'skzrealm',
  'Stray Kids members deep knowledge': 'skzrealm',
  'K-pop iconic dances - name that move': 'skzrealm',
  'K-pop variety show moments': 'skzrealm',

  // soojinnie - aespa, (G)I-DLE
  'aespa MY concept universe': 'soojinnie',
  '(G)I-DLE Tomboy era quiz': 'soojinnie',
  'K-pop songs with colors in the title': 'soojinnie',
  'K-pop positions explained quiz': 'soojinnie',

  // caratland - SEVENTEEN
  'SEVENTEEN FML era quiz': 'caratland',
  'SEVENTEEN members nationalities': 'caratland',
  'K-pop albums that sold millions': 'caratland',

  // njeansstan - NJ, IVE, LSRFM
  'NewJeans Hype Boy vs Attention quiz': 'njeansstan',
  'IVE After LIKE to I AM era': 'njeansstan',
  'LE SSERAFIM ANTIFRAGILE to UNFORGIVEN': 'njeansstan',
  'K-pop songs of 2023 quiz': 'njeansstan',
  'K-pop songs of 2024 quiz': 'njeansstan',

  // twiceland - ITZY
  'ITZY discography deep dive': 'twiceland',
  'K-pop food and mukbang culture': 'twiceland',
  'Complete the K-pop song title': 'twiceland',

  // exoplanet99 - EXO + general
  'EXO discography challenge': 'exoplanet99',
  'EXO members and positions': 'exoplanet99',
  'K-pop 2nd generation legends': 'exoplanet99',
  'MAMAMOO members and facts': 'exoplanet99',
  'MAMAMOO true or false': 'exoplanet99',
  '2NE1 legacy quiz': 'exoplanet99',

  // kpophistory - general K-pop
  'K-pop idols in Korean dramas': 'kpophistory',
  'K-pop idols in dramas true or false': 'kpophistory',
  'K-pop debut songs - name the group': 'kpophistory',
  'K-pop debut songs true or false': 'kpophistory',
  'K-pop group leaders quiz': 'kpophistory',
  'K-pop maknae (youngest member) quiz': 'kpophistory',
  'IU - the nation\'s little sister': 'kpophistory',
  'PSY and Gangnam Style era': 'kpophistory',
  'K-pop fun facts true or false - volume 2': 'kpophistory',
  'K-pop English songs quiz': 'kpophistory',
  'K-pop number ones - Billboard Hot 100': 'kpophistory',
  'SM Entertainment groups quiz': 'kpophistory',
  'JYP Entertainment groups quiz': 'kpophistory',
  'YG Entertainment groups quiz': 'kpophistory',
  'HYBE groups quiz': 'kpophistory',
  'K-pop numbers quiz - how many members?': 'kpophistory',
  'Name the group from the fandom name': 'kpophistory',
  'K-pop birthplace quiz - where are they from?': 'kpophistory',
  'K-pop birthplace true or false': 'kpophistory',
  'TREASURE basics': 'kpophistory',
  'Kep1er quiz': 'kpophistory',
  'K-pop collaboration with Western artists': 'kpophistory',
  'K-pop lightstick colors and shapes': 'kpophistory',
  'K-pop Weverse and fan platforms': 'kpophistory',
};
...
async function main() {
  ...
  // Load quiz data
  const quizData = JSON.parse(fs.readFileSync('scripts/batch2-quizzes.json', 'utf-8')) as Array<{
    title: string;
    group: string;
    type: string;
    difficulty: string;
  ...
  console.log(`Loaded ${quizData.length} quizzes from batch2-quizzes.json`);
  ...
  // ---- Step 1: Look up existing batch2 creator profiles ----
  console.log('\n1. Loading existing creator profiles...');
  const userMap = new Map<string, string>(); // username -> uuid
  const batch2Usernames = CREATORS.map(c => c.username);
  // Also load seed-platform users to use as additional creators
  const allKnownUsernames = [...batch2Usernames, 'army_mina97', 'jimin_universe', 'blink_forever22', 'stay_dreamer', 'carat_hoshi', 'once_upon_sana', 'my_winter99', 'bunny_haerin', 'exol_chen', 'midzy_ryujin', 'kpop_scholar', 'hallyu_nerd'];
  ...
  if (userMap.size === 0) {
    console.error('  No creators found! Run import-batch2.ts first.');
    return;
  }
  ...
  // Update group stats (recalculate for ALL groups, not just batch2)
  ...
  // Update profile stats for batch2 creators
  ...
  // Update profile total_likes_received for batch2 creators
  ...
  // Top batch2 quizzes
  const batch2Ids = quizRecords.map(q => q.id);
