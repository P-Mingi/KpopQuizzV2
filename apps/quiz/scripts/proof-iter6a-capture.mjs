// Iteration 6 PART A proofs: grey nav vs white content separation, full-bleed (no cream),
// the light top block - home + one index + one member page, open sidebar + icon rail, light + dark.
//   node scripts/proof-iter6a-capture.mjs <baseUrl> <slug> <outDir>
import { spawn } from 'node:child_process';
import { writeFileSync, mkdirSync } from 'node:fs';

const BASE = process.argv[2] ?? 'http://localhost:3021';
const SLUG = process.argv[3] ?? 'bts';
const OUT = process.argv[4] ?? 'docs/proofs/v3nav-iter6a';
mkdirSync(OUT, { recursive: true });

const chrome = spawn('/Applications/Google Chrome.app/Contents/MacOS/Google Chrome', ['--headless=new','--remote-debugging-pipe','--disable-gpu','--hide-scrollbars','--force-device-scale-factor=1','--window-size=1440,900','--user-data-dir=/tmp/iter6a-prof','--no-first-run','--no-default-browser-check','about:blank'], { stdio:['ignore','inherit','inherit','pipe','pipe'] });
...
async function main(){
  ...
  const theme=async v=>{await evalJs(`(()=>{const r=document.documentElement;r.classList.remove('light','dark');r.classList.add('${v}')})()`);await sleep(300)};
  const shot=async(name,full)=>{const h=full?Math.min(await evalJs('Math.max(document.body.scrollHeight,document.documentElement.scrollHeight)'),2600):860;const{data}=await send('Page.captureScreenshot',{format:'png',captureBeyondViewport:!!full,clip:{x:0,y:0,width:1440,height:h,scale:1}},S);writeFileSync(`${OUT}/${name}.png`,Buffer.from(data,'base64'));console.log('saved '+name)};
  const goto=async u=>{await send('Page.navigate',{url:u},S);await sleep(2000)};

  await goto(`${BASE}/verse/${SLUG}`); await theme('light'); await shot('home-open-light',false);
  await theme('dark'); await shot('home-open-dark',false); await theme('light');
  await evalJs("document.getElementById('v-nav-collapse').checked=true"); await sleep(300); await shot('home-rail-light',false);
  await goto(`${BASE}/verse/${SLUG}/discography-index`); await theme('light'); await shot('index-open-light',false);
  await goto(`${BASE}/verse/${SLUG}/jungkook`); await theme('light'); await shot('member-open-light',false);
  await theme('dark'); await shot('member-open-dark',false);
