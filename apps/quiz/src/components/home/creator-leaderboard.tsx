...
export function CreatorLeaderboard({ creators }: CreatorLeaderboardProps): React.ReactElement {
  ...
  return (
    <div className="border-t border-border-light pt-6 mt-6">
      <p className="text-sm font-medium text-txt-secondary mb-3">Top quiz creators this week</p>

      <div>
        {creators.map((creator, i) => (
          <div key={creator.username} className="flex items-center gap-3 py-2">
            <span className="text-xs text-txt-tertiary min-w-[16px]">#{i + 1}</span>
            <UserAvatar
            ...
            <Link
              ...
              className="text-sm font-medium text-txt-primary hover:underline"
            >
              {creator.username}
            </Link>
