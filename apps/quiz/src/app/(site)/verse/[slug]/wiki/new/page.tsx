      ...
      {allowed ? (
        ...
        <div style={{ maxWidth: '46rem' }}>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            {!userId ? (
              <Link href={`/login?returnTo=${encodeURIComponent(`/verse/${space.group.slug}/wiki/new`)}`} className="inline-flex min-h-[44px] items-center rounded-xl px-5 text-sm font-bold no-underline" style={{ background: 'var(--verse-cta, var(--verse-accent))', color: 'var(--verse-cta-text, #fff)' }}>Sign in</Link>
            ) : null}
