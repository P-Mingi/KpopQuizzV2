...
// W5.3 - merch + lightstick gallery. GET is public (published items). POST/PATCH are curator
// entry; images are strict-legal (approved hosts only), otherwise stored null.
...
export async function POST(req: NextRequest): Promise<NextResponse> {
  ...
  const { data, error } = await svc.from('collectibles').insert({ ...patch, group_id: groupId, status: body.publish ? 'published' : 'draft', created_by: uid }).select('id').single();
