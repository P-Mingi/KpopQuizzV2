...
export async function GET(req: NextRequest): Promise<NextResponse> {
  ...
  const { data: resultRows } = await db
    .from('battle_results')
    .select('battle_id, player_hash')
    .in('battle_id', candidates.map((b) => b.id));

  const players = new Map<string, Set<string>>();
  for (const r of (resultRows ?? []) as { battle_id: string; player_hash: string }[]) {
    const s = players.get(r.battle_id) ?? new Set<string>();
    s.add(r.player_hash);
    players.set(r.battle_id, s);
  }

  const open = candidates.filter((b) => {
    if (b.challenger_hash === playerHash) return false; // never your own run
    const played = players.get(b.id);
    if (!played) return true;
    if (played.has(playerHash)) return false; // you already answered this one
    // Open only while nobody but the challenger has run it.
    return [...played].every((h) => h === b.challenger_hash);
  });
  ...
  open.sort((a, b) => {
    if (groupSlug) {
      const ga = a.group_slug === groupSlug ? 0 : 1;
      const gb = b.group_slug === groupSlug ? 0 : 1;
