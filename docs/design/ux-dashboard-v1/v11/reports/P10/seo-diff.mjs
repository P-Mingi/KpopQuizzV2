...
function extract(html) {
  ...
  // The page's own content (the shell's nav, tab bar and footer are A0's scope): the
  // route streams behind its loading.tsx, so the page arrives after the shell in
  // <div hidden id="S:n"> segments; without streaming it is inside <main>.
  const si = body.search(/<div hidden="" id="S:\d+"|<div hidden id="S:\d+"/);
  const mi = body.search(/<main\b/i); const me = body.lastIndexOf('</main>');
  const mainHrefs = si >= 0 ? linkSet(body.slice(si)) : mi >= 0 && me > mi ? linkSet(body.slice(mi, me)) : hrefs;
  return {
