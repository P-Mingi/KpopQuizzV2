...
- Done (branch `ux11/p11-notifications`, merged with `origin/feat/ux-v1-v11` 89e1e94 = A0's queue PR #50):
  ...
  clean; report images; request file `v11/requests/P11.md`; flag-off builds + diff (8 guest pages and signed-in
  /notifications IDENTICAL); SEO + link-set diff (no difference, 0 lost links); page cases also green on a flag-on
  production build; data safety (test user's rows unchanged). Cleanup done (servers, builds, symlinks, .next, auth
  state; A0's two files restored). PR #53 into `feat/ux-v1-v11`: https://github.com/P-Mingi/KpopQuizzV2/pull/53
- Next: A0 swaps the slot exports after this PR merges (request 1) and adds Mark all read to the bell header
  (request 2); then the overlay and bell e2e cases stop skipping on the integration branch.
- Blockers: none.
...
| before (flag off) / after (flag on) / prototype | ![](P11/before-after-prototype-notifications-1440-light.webp) | ![](P11/before-after-prototype-notifications-1440-dark.webp) | ![](P11/before-after-prototype-notifications-390-light.webp) | ![](P11/before-after-prototype-notifications-390-dark.webp) |
...
flag-on PRODUCTION build (next start, no slot swap): 20 passed, 22 skipped (overlay + bell: slot not wired),
  1 flaky (auth over 2.5 s: /api/auth/me fails soft to no profile, so no streak row on the first attempt)
```
...
- `vitest run src/lib/ux-v1/p11`: 31 passed (categories, icons for every live type, targets incl. javascript: /
  ...
  safety, relevance, merges, labels). `vitest run src/lib/ux-v1` after merging 446f161: 13 files, 369 passed.
- Whole-app `pnpm exec tsc --noEmit -p .` (e2e included) after merging `origin/feat/ux-v1-v11` 446f161, A0's two
  files restored: 1 error, NOT in a P11 file: `e2e/ux-v1/p5.spec.ts(783,7)` (`routes: (p) => p.route(...)` returns
  `Promise<Disposable>`, typed `Promise<void>`), identical on the integration head (P5's merge). P11 files: 0 errors
  (also 0 before that merge). eslint-config-next (core-web-vitals + typescript) on every P11 file: 0 problems.
...
- Bell and overlay on a production build or a Vercel preview: the flag-on production build of this branch has no
  swap (its page cases pass there, 4.1); C3 re-runs everything on the preview once A0's swap is merged.
