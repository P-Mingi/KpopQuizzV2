...
- 2026-09-26, request queue: done on `ux11/a0-foundation` (queue PR into `feat/ux-v1-v11`, section 8).
  ...
  f11ec96 + f068e25; streak test time bomb 5bfe147. Replies with the shas in `v11/requests/A0.md` (the guard keeps A0
  out of `requests/P4.md`, `P10.md` and `P1.md`).
...
- Next: ORCH copies the replies into the request files and merges the queue PR.
- Blockers: none.
...
- Specs on a flag-on production build of this branch (`next build` + `next start`; the dev server on this
  ...
  kit.spec + shell.spec at f11ec96, 1440 and 390, light and dark: 51 passed, 5 skipped (width-specific
  cases), 1 transient (`/api/auth/me` fell back to signed out after its 2.5 s auth timeout on the loaded
  machine; passed on rerun). Re-run on the final head: pending.
