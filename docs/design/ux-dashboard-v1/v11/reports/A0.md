...
- Done: ownership guard + per-worktree hook (proven); v11.2 tokens (light / dark warm, 17.10 light lines);
  top-nav shell (pink pill + icons + Home, search and bell slots, streak popover, account menu), phone tab bar,
  footer; every shared component (buttons, links, cards, post card, panels, tabs / segmented / chips / dropdown,
  four sheets, toast + live region, PersonName + BiasTag, BadgeMedal, stats row, pinned row, form controls);
  UxQuizCard v11.2 moved to `components/ux-v1/quiz-card.tsx` (both delegators point at it, old file deleted);
  `/ux-v1/kit`; `auth.setup.ts` + Playwright projects; `shell.spec` + `kit.spec` green at 1440 and 390, light
  and dark; flag-off proof (structure identical, CSS byte-identical, JS lighter than base).
- Next: ORCH merges after the allowlist line below; page agents build on the API list (section 5).
- Blockers (ORCH decision, file owned by nobody): `apps/quiz/src/lib/route-allowlist.ts` must list `'/ux-v1/'`
  (kit) and `'/community'` (P8). Without the first line, `pnpm --filter quiz build` fails at `check:routes` on
  this branch and the middleware 301s the kit to `/`. Patch in section 6.1.
...
- `@/components/ux-v1/segmented` (client): `Segmented({ options: { value, label }[], value, onChange, label })`.
- `@/components/ux-v1/chip`: `Chip({ children, filter?, pressed?, onRemove?, onClick?, href?, icon? })`, `Chips({ label? })`.
- `@/components/ux-v1/dropdown` (client): `UxDropdown({ label, value, options, onChange })`.
...
- `@/components/ux-v1/share-sheet`: `ShareSheet({ open, onClose, title?, preview?: { image?, line1, line2? }, url, text?, onStoryImage?, storyFile?, challenge?: { url, note?, help? } })`.
...
- `@/components/ux-v1/use-ux-me`: `useUxMe()`, `refreshUxMe()` (after a quiz / blindtest: streak pill and popover switch to "saved").
