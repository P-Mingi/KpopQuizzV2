...
- 2026-09-26, round 3 (P5 and P11 merged; section 9): P11 1 slots render P11's SearchResults and BellPanel
  03ac6b6; P11 2 Mark all read in the bell header a091d1c; P11 4 streak popover states the real rule 3c85a64;
  P11 5 search field without the ring box and the native clear button 8cb7491 + 0ae0e19; P5 2 sheet title
  18/1.6 5065b55; P5 3 UxQuizCard preview mode 53f1230. Replies in `v11/requests/A0.md` (de2700c).
- Rounds 1 and 2 (PR #50, merged 7b95070): section 8. Foundation (PR #45, 2cad6a7): sections 1 to 7.
- Next: ORCH merges PR #56 (round 3) and copies the replies; P5 can drop `DraftCard` and its sheet-title skip.
- Note for ORCH: b41bbcc is ORCH's f9e47e3 with only its message replaced (same tree, same parent): the round
  3 merge of the integration branch was a fast-forward, and A0's usual "amend the merge message" step then
  amended ORCH's commit. It changes nothing (RUN-STATE.md equals the integration branch, net diff 0), but the
  guard's per-commit range check lists RUN-STATE.md because of it. Left as is (no force push).
- Blockers: none.
...
- tsc 0 errors. eslint (eslint-config-next core-web-vitals + typescript) on every changed file: 0 problems.
  Unit: `vitest run` 633/633 on the final code (363/363 before P1 and P7 merged). Em / en dash: none in the
  changed files.
- Specs on a flag-on production build of this branch (`next build` + `next start`; the dev server on this
  ...
  kit.spec + shell.spec on the final code (both batches, 5de7fe3), 1440 and 390, light and dark: 62 passed,
  5 skipped (width-specific cases), 0 failed; the kit's two-line stacked row measures 350 x 118.781, the
  reference exactly. (Earlier: f068e25 54 passed; f11ec96 51 passed + 1 transient where `/api/auth/me` fell
  back to signed out after its 2.5 s auth timeout on the loaded machine, green on rerun.)
- Consumers (whole spec files, flag-on production builds):
  - P4 + P6 + P10 on f11ec96: 186 passed, 1 skipped (P4's phone-only case at 1440), 0 failed (187 tests).
  - P1 + P4 + P6 + P7 + P10 on 348fdca (every item of both batches, P1 and P7 merged): 256 passed, 1 skipped
    (the same P4 phone-only case), 0 failed (257 tests, 7.4 min).
  - P3 (merged after the batches; its hub uses `Segmented` and `UxDropdown`) + kit + shell on 897fe8f: 96 passed,
    5 skipped, 0 failed.
  - A run on f068e25 in between failed 39 tests while production Supabase timed out (the middleware logged
    "timeout after 2500 ms - failing open" 178 times; P6's hub islands never went live; one P4 like count
    moved under the test): environment, not code; the 348fdca run above is green on the same specs.
...
```
After P3 merged (3d8b56a, merged in ba07548), same check on its URLs: `/groups` 85 -> 144, `/bts-quiz`,
`/blackpink-quiz`, `/twice-quiz`, `/blackpink-trivia`, `/bts-trivia` lost 0, `/search` present on all. (A
build made while production Supabase timed out prerendered `/groups` and the home with their fallbacks and
showed 55 and 7 lost links; the rebuild with one timeout above is the one measured. Build-time data, not code.)

### 8.4 Flag off

Both trees built with the flag off back to back (`next build`, 804 / 805 static pages: the extra one is the
flag-on-only `/ux-v1/kit/focus`), served with `next start` (:4001 = `origin/feat/ux-v1-v11` fd99234, :4002 = this
branch at 348fdca), diffed with `e2e/ux-v1/helpers/flag-off-diff.mjs --structure` as in section 3.3:
```
IDENTICAL (structure)  /  /quizzes  /bts-quiz  /blindtest  /groups  /pt/blindtest
IDENTICAL (structure)  /q/skz-true-or-false-only-real-stays-pass  /q/blackpink-true-or-false
                       /q/stray-kids-basics  /q/one-for-sones
DIFFERENT              /q/ultimate-bts-era-quiz-only-real-armys-survive: the stats line and list only
```
The one difference is data, not code: the branch build's prerender of that quiz logged `getQuizExtraStats
failed ... fetch failed` (3 times, production Supabase), so it printed the no-stats variant of the copy ("have
taken it, averaging 52%") where the base printed the perfect-score variant; the page code is unchanged and the
four other quiz pages are identical.
Stylesheets byte-identical (2 files, 396,253 + 8,737 B). JS per page, flag off: `/` 709,905 / 709,906 B,
`/quizzes` 712,278 / 712,237, `/q/<slug>` 1,001,745 / 1,001,747, `/bts-quiz` 713,551 / 712,809, `/blindtest`
717,348 / 717,347, `/groups` 686,551 / 685,848 (base / branch, same file counts). Flag-on-only routes answer
the same flag off on both: `/ux-v1/kit` and `/ux-v1/kit/focus` 301 to `/`, `/api/ux-v1/a0/styles` 404,
`/blindtest/ranked` 404; `/search` 200.

## 9. Round 3 (2026-09-26; P5 and P11 merged, base 8934db9)

| Item | Change | Sha | Proof |
|---|---|---|---|
| P11 1: swap the slots | `slots.tsx` re-exports P11's `SearchResults` / `BellPanel` (+ prop types); A0 keeps triggers, frames, focus, closing | 03ac6b6 | `p11.spec` overlay and bell cases run (0 skipped); shell.spec's overlay case checks the host (slot body under the field, query kept) |
| P11 2: Mark all read | bell header `button.ux-lnk` while `unread > 0`: `markAllRead()` (live POST `/api/notifications/mark-read` `{}`, unread store 0, P11 read event) + toast "All marked as read"; `.ux-pop-h .ux-lnk` 14px / 1.6, flex (prototype `#bellpop .pop-h .lnk`) | a091d1c | `p11.spec` "bell: Mark all read in the header (A0 request 2)": one stubbed POST `{}`, every row read, the bell label drops the count; the bell landmarks include the link |
| P11 4: streak rule | at risk: "Play the daily quiz or the daily blindtest in the next {left} to keep it." (the backend counts only those two; same rule as P11's streak row) | 3c85a64 | kit.spec "streak popover: the real rule" |
| P11 5: search field | `#ux-sq`: no outline box, no `::-webkit-search-cancel-button` / decoration (`type="search"` kept); 16.9 focus: while the field has focus the row's hairline becomes a 2px pink line (1px border + 1px inset shadow, same geometry), back to the hairline when focus moves to the rows | 8cb7491, 0ae0e19 | kit.spec: outline none; the focused filled field paints exactly as the same field as `type="text"` (no button) while a forced native button makes the paint differ (control); pink 2px line on focus, gone after Tab |
| P5 2: sheet title | checked in the prototype source: `.sh-h h3{font-size:18px;font-weight:600}` inherits body 16px/1.6 and h1-h3 get -0.015em; `.ux-sh-h h2/h3` now 18px / 1.6 / -0.015em (were 1.4 / -0.01em) | 5065b55 | kit.spec: 18px, 28.8px, -0.27px, 600, box 29px |
| P5 3: UxQuizCard preview | `preview` (+ `previewLabel`): `<div role="img" aria-label="Preview: title, group, level">`, no link; a `data:image/` or `blob:` cover as a plain `<img>` (preview only); `.ux-qcard.is-preview` has no hover lift | 53f1230 | kit preview card (data: SVG cover) + kit.spec: DIV, role img, named, no link, data: img, "New", same border / radius / background / padding / width as a linked card, no hover change |

Checks (flag-on production build of 53f1230; spec files at 0ae0e19):
- tsc: only P5's known p5.spec(783) error before 8934db9, 0 after merging it. eslint 0 on the changed files;
  unit 738/738; no em / en dash.
- kit + shell + p11: 112 passed, 5 skipped (width-specific A0 cases; p11 skipped nothing). The 2 search-field
  cases first failed on a check that cannot work (getComputedStyle does not read the cancel-button pseudo);
  with the paint proof (0ae0e19) they pass, and the whole kit passes again in the next run.
- p1, p3, p4, p5, p6, p7, p10 + kit: 375 passed, 1 skipped (P4's phone-only case at 1440); 1 failed and green
  on rerun (P4 results at 390: `GET /api/quiz/[id]/questions` did not answer within 90 s, live DB).
- Server HTML link sets are not touched by this round (slot bodies, bell header and sheets render on the
  client only; the nav search link is unchanged).

Flag off (base 8934db9 on :4001, this branch on :4002, both `next build` with the flag off, 808 static pages
each): with `flag-off-diff.mjs --structure --out` and digit runs masked (live play counts moved between the
builds), 12 of 13 pages are identical (`/`, `/q/<4 slugs>`, `/bts-quiz`, `/blindtest`, `/groups`, `/create`,
`/trending`, `/faq`, `/pt/blindtest`); `/quizzes` differs only in the order of two quiz cards (sorted by live
plays). Unmasked, the differences are counts only (plays, "420+" vs "380+" quizzes from a count read); the
branch's first flag-off build had 23 Supabase fetch failures and prerendered two quiz pages with their
fallback, so it was rebuilt (2 failures, on a page not compared). Stylesheets byte-identical (396,253 +
8,737 B). JS +32 to +441 B per page (the UxQuizCard preview branch is in the shared card chunk). Flag-on-only
routes answer the same on both: `/ux-v1/kit` and `/ux-v1/kit/focus` 301, `/api/ux-v1/a0/styles` and
`/api/ux-v1/p11/search` 404; `/notifications` 307 to `/login`; `/search` 200.

## 10. Fix loop 1 (2026-09-26; base e546718)

| Issue | Fix | Sha | Proof |
|---|---|---|---|
| C2-001: `GET /api/quizzes/count` answered 200 with the flag off | returns 404 `{"error":"not_found"}` unless `NEXT_PUBLIC_UX_V1` is on (same pattern as `/api/ux-v1/p11/search`); flag on unchanged. No caller on any ux11 branch (P2's local branch included): the owner may delete it in the cleanup | 441c418 | flag-off `next build` + `next start`: `/api/quizzes/count` 404 `{"error":"not_found"}` (with `/api/ux-v1/p11/search` 404, `/api/ux-v1/a0/styles` 404, `/ux-v1/kit` 301, `/` 200); flag-on build: 200 `{"count":425}` |
| C1-002: the header picture sheet opened with the drop zone in its hover look | the sheet moves focus into the zone's file input; `.ux-drop:focus-within` painted the hover look. The hover look is now `:hover` only (prototype `.drop:hover`); keyboard focus keeps the 16.9 ring through `.ux-drop:has(input:focus-visible)` (2px pink); the focus move stays | d07e6c2 | kit.spec "header picture sheet: the drop zone opens at rest": opened by pointer and measured without blurring, dashed `--edge` border, no fill, no ring, focus inside the zone; opened by keyboard, 2px solid ring and no fill; hover still shows the hover look. p10.spec (incl. its header-sheet state vs P10's prototype capture) green |

Checks: whole-app `pnpm exec tsc --noEmit -p .` 0 errors; eslint 0 on the changed files; kit.spec + p10.spec on a
flag-on production build: 129 passed, 0 failed (1440 and 390, light and dark). P10's spec still blurs before
measuring; with this fix it no longer needs to (P10 may drop that workaround).
