...
- PR #51 into `feat/ux-v1-v11`: https://github.com/P-Mingi/KpopQuizzV2/pull/51 (merges cleanly on fd99234).
- Cleanup done: every P3 server stopped (dev :3033, builds :4331 / :4332 / :4333), build copies, `.next`,
  `test-results`, the test-user storage state and the worktree symlinks removed.
...
Prototype styles: every landmark comparison in the spec (`compareLandmarks` against `styles.json`: `.nav`,
`.links a.on`, `.sec-h h2`, `.btn-primary`, `.tcard` on the states groups, hub-blackpink, hub-ateez, hub-empty)
asserts `missing = []` and `mismatches = []`: 0 mismatches at 1440 and 390, light and dark, in both final runs.
...
- Empty hubs follow the prototype (Make the first quiz + Notify me, no Blindtest button) even when the group has a
  playable blindtest (Chungha: "18 blindtest songs" in the facts). A Blindtest button there would link the legacy
  mode page (bug above); add it once the v11 deep link exists, if the owner wants it.
