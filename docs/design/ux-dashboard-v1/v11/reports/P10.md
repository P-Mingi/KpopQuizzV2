...
- Done (branch `ux11/p10-passport`, on `feat/ux-v1-v11`): header routes (upload + link, SSRF-safe, fail soft
  ...
  `p10.css`, pending migrations (bucket, email opt-ins), unit + render tests, `e2e/ux-v1/p10.spec.ts`, prototype
  style reference for the passport / settings / header-sheet landmarks, flag-off diff, SEO diff, report shots.
- Next: ORCH review and merge; after the owner answers, C2 can verify signed-in `/me` and real header uploads.
