...
`origin/feat/ux-v1-v11` (dc863a8 + docs) and of this branch, served with `next start` on :4101 (base) and :4102
(P10), fetched back to back with A0's `e2e/ux-v1/helpers/flag-off-diff.mjs` (GET only, as a guest):
...
IDENTICAL (structure)  /settings         status 307/307                            js chunks 0/0
...
IDENTICAL (structure)  /                 status 200/200  normalised 142749/142749  js chunks 13/13
IDENTICAL (structure)  /u/testtest       status 200/200  normalised 32612/32612    js chunks 13/13   (build of bed08c8)
IDENTICAL (structure)  /quizzes          status 200/200  normalised 268532/268532  js chunks 13/13   (build of bed08c8)
```
...
- JS weight flag off (every chunk the HTML references): `/u/<name>` base 750,570 B, P10 751,661 B (+1,091 B:
  the tiny next/dynamic loader of the islands); `/` +14 B. A first build without the loaders showed +30,709 B on
  `/u`: the flag-gated `await import()` of the passport still put its client islands (and the A0 sheets they use)
  into the route's chunk graph; `components/profile/ux-v1/islands.tsx` fixed it (A0's shell pattern).
- The /u/testtest, /u/mingi and /u/stanloona entries of the second P10 build were rendered while Supabase was
  answering slowly (`getProfileByUsername timed out after 5000ms - using fallback` in the server log): they show
  the generic fallback head on that build only. The base build shows the same fallback for an uncached profile
  at the same time (/u/h_aeyie), so it is the environment, not the code; the same page was IDENTICAL on the
  first P10 build (bed08c8, whose flag-off tree is the same as HEAD's: later commits only touched modules that
  are imported when the flag is on, plus the loaders measured above).
- Flag off, the header routes and the export route answer 404; `/api/ux-v1/a0/styles` (and so `p10.css`) is not
  linked; `/settings` renders the legacy page (the v11 settings is a lazy chunk created only when the flag is on).
