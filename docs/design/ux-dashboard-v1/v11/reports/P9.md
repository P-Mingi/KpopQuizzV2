...
6. Real-data finding outside this run (flag off, live today): `/pt/leaderboard` pads its weekly creators board
   with made-up fan accounts (`lib/weekly-leaderboard-padding.ts`, FAKE_USERS; 3 fake rows today because only 3
   real creators published this week). Unowned; flagged for a separate fix.
