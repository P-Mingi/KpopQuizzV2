...
- Done: `/leaderboard` v11 behind `NEXT_PUBLIC_UX_V1` (flag off: `CommunityContent` exactly as before): tabs Fandom
  ...
  page is kept. Unit + render tests 29/29 (full suite 677/677), e2e `p9.spec.ts` 21/21 at 1440 and 21/21 at 390
...
e63e092): 0 errors. eslint-config-next (core-web-vitals + typescript) on the 19 P9 .ts / .tsx files: 0 problems. vitest: the P9
files 29/29, the whole suite 677/677. `check:routes` 330 reachable (329 on the base: the new API route). Production
...
- Read timeouts in production: like the live page (safeFetch, 5 s, ISR 5 min), a regeneration whose reads time
  out bakes the fallback for one ISR window. Seen locally under a load average of 30 to 40 (the build's own
  prerender and one regeneration); the page then says the board could not load (above). Vercel's timing is not
  measured here.
