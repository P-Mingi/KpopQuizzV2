...
- Done: `/leaderboard` v11 behind `NEXT_PUBLIC_UX_V1` (flag off: `CommunityContent` exactly as before): tabs Fandom
  ...
  whole-app tsc 0, eslint 0, vitest 769/769, axe 0 serious / critical. Merged `origin/feat/ux-v1-v11` 8934db9
  (docs only since b3d9e36). ORCH's daily debate check: the flag-on page drops the `ensure_daily_debate` call (owner
  decision 10); no write path added.
...
| Tests | `src/lib/ux-v1/p9/p9.test.ts` (15), `src/components/leaderboard/ux-v1/render.test.ts` (16), `e2e/ux-v1/p9.spec.ts` (20 cases x 2 widths + setup) |
...
| Row | Wired to | Proof |
|---|---|---|
| Fandom war podium + board + weekly change | the war map RPC `get_fandom_war_map` with the live `getFandomWarMap` mapping (1 h cache, 90 groups read, top 30 shown as the live page) but a failed read throws (`getWarMap`, section 9.9); fandom names from `groups.fandom_name`; the change = the live percent vs the 7 days before | e2e (hub links = the live RPC read), render test |
...
Re-run on the final build (15:15Z, settled pages): SEO identical, 0 lost on both; `/leaderboard` 76 -> 97 (the
flag-off count moves with the live feed's distinct profiles), `/pt/leaderboard` 25 -> 41. Added on `/leaderboard`: the v11 shell's (`#main`, `/groups`, `/community`, `/daily`, `/blindtest/ranked`,
...
files 31/31, the whole suite 769/769 (after merging b3d9e36). `check:routes` 331 reachable after the merge (the P9 API route adds one: 329 -> 330 against the 89e1e94 base). Production
...
7,128, creator_notifications 1,210 -> 1,211: live site traffic; the other tables unchanged). Final run 15:15Z, after
the last e2e runs (`P9/proofs/data-guard-final.txt`): every count equal or up again (plays 67,863, activity_events
7,227, profiles 260, quizzes 437: live traffic), no Newest moved backwards. The parity test user's
...
9. Finding in shared code (unowned, live today): `getFandomWarMap` (`lib/db/queries/community.ts`) maps an RPC
   error to an empty board and `unstable_cache` keeps it for an hour. Seen during this run: a Supabase connection
   timeout at build time gave "No fandom has points this week" for the hour. The live `/leaderboard` then hides its
   war map (and its 30 hub links), the group hubs lose their war line, the passport its war strip. P9 reads the
   same RPC through its own `getWarMap`, which throws instead (not cached; the page says the board could not load).
   A fix in the shared function (throw, or skip caching an empty result) is an owner call.
10. The daily debate stops rotating from `/leaderboard` when the flag goes on (checked for ORCH / P8). The rotation
    is a write on view: `getDailyDebate()` (`lib/db/queries/debate.ts`, service role) calls
    `rpc('ensure_daily_debate')`, and its only caller is the legacy `components/community/community-content.tsx`
    (the flag-off `/leaderboard` and `/verse/community`, each on each ISR regeneration). The flag-on `/leaderboard`
    does not mount `CommunityContent` and makes no debate call. What remains with the flag on: `/verse/community`
    still mounts it (unchanged by P9), but that mirror is out of the sitemap, canonical to `/leaderboard`, and 302s
    guests to `/verse` while Verse is hidden (`isGatedVersePath`), so it regenerates only on rare signed-in visits.
    No cron calls it (`apps/quiz/vercel.json`, no route under `api/cron` mentions the debate), and P1's home reads the debate read-only. So, flag on, the daily debate
    effectively stops rotating. P9 added no write path (the page stays a pure read). Owner call: a scheduled
    rotation (e.g. a Vercel cron calling `ensure_daily_debate`), or keep a rotation call on some flag-on page.
